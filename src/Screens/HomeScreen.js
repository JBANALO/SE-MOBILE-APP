...
export default function HomeScreen() {
  ...
  const sendEmailToParent = (student, period) => {
    ...
    if (!student.parentEmail) {
      Alert.alert('No Email', 'Parent email not available for this student.');
      return;
    }
...
This is to inform you that ${student.name} (${student.studentId}) was marked as ${status} for the ${period} session today.
...
${teacher.department}`;
    ...
    const mailtoUrl = `mailto:${student.parentEmail}?subject=${encodeURIComponent(subject)}&body=${encodeURIComponent(body)}`;
    
    Linking.openURL(mailtoUrl).catch(() => {
      Alert.alert('Error', 'Could not open email app. Please check your device settings.');
    });
  };
